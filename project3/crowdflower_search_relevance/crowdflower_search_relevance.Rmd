...
3. **Writing and Benchmarking Various Machine Learning Algorithms.**: In this part I am going to try out several ML algorithms and benchmark them based on their performance and precision. Several submissions may be required here to find out how well the model generalise on different dataset.
...
In our project, let us try both stemming and lemmatization and see which of them produces better model. Stemming algorithm is provided by R's Text Mining package (`package(tm)`) while Lemmatization algorithm is provided by **TODO** package.

**Trying out Lemmatization Package**

Stemming algorithm is relatively simple, but I would probably need to test out Lemmatization algorithm to see if it behaves as expected.
...
### Google Early Paper on Search Engines
...
```{r}
# This function stems the texts found in "var" variable of dataframe "df"
# and returns the passed dataframe "new_df" with "[var]_stemmed" and "[var]_stemmed_count"
# variables.
stem_corpus <- function(df, new_df, var) {
  ...
  corpus_text <- data.frame(text=unlist(
    sapply(corpus, `[`, "content")), stringsAsFactors=F)
  eval(parse(text=paste("new_df$", var,
                        "_stemmed <- corpus_text$text", sep="")))
  eval(parse(text=paste("new_df$", var,
                        "_stemmed_count <- sapply(",
    "gregexpr(\"\\\\W+\", corpus_text$text), length) + 1",
    sep="")))
  return(new_df)
}
...
preprocess_data <- function(df) {
  ...
  new_df <- stem_corpus(df, new_df, "query")
