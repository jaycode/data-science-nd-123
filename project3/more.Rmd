...
#### 5.1.5. Setting Up Score Calculation Helpers and Initial Plots

Let us start by running a simple scoring system and add score to each row in our dataset.
...
p1 <- ggplot(aes(x = train$median_relevance, y = train$title_simple_score), data = train) +
  geom_jitter(alpha=.2)
p2 <- ggplot(aes(x = train$relevance_variance, y = train$title_simple_score), data = train) +
  geom_jitter(alpha=.2)

p3 <- ggplot(aes(x = train$median_relevance, y = train$description_simple_score), data = train) +
  geom_jitter(alpha=.2)
p4 <- ggplot(aes(x = train$relevance_variance, y = train$description_simple_score), data = train) +
  geom_jitter(alpha=.2)
